...
class OrtonConeReference:
    def __init__(self):
        self.cone_data = {
            # Cone number: (Temperature, Time)
            "022": (600, 30),  # Example values
            # ... add more cones
        }
...
class ConeModeController:
    def __init__(self, oven):
        self.oven = oven
        self.is_cone_mode_active = False
        self.cone_mode_timer = 0
        self.cone_mode_temperature_drop = 30  # Degrees Celsius
        self.orton_cones = OrtonConeReference()

    def activate_cone_mode(self, cone_number):
        if cone_number in self.orton_cones.cone_data:
            self.is_cone_mode_active = True
            temp, time_extension = self.orton_cones.cone_data[cone_number]
            self.cone_mode_timer = time_extension
            self.adjust_temperature(temp)
            self.extend_timer(time_extension)
    ...
    def adjust_temperature(self, target_temp):
        # Adjust the temperature of the oven
        current_temp = self.oven.get_current_temperature()
        new_temp = current_temp - self.cone_mode_temperature_drop
        self.oven.set_temperature(new_temp)

    def extend_timer(self, time_extension):
        # Extend the oven's operating timer
        self.oven.extend_timer(time_extension)

    def check_timer(self):
        # Check if the cone mode timer has expired
        if self.cone_mode_timer <= 0:
            self.deactivate_cone_mode()
        else:
            self.cone_mode_timer -= 1  # Decrement the timer
