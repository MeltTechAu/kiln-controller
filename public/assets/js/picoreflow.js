...
// Cone Mode Feature Logic
$(document).ready(function() {
    // Event listener for the Set Cone Mode button
    $('#set-cone-mode').click(function() {
        var selectedCone = $('#cone-selection').val();
        // Send request to backend to activate cone mode
        activateConeMode(selectedCone);
    });
});

function activateConeMode(coneNumber) {
    // TODO: Implement the logic to send request to backend
    // Example: ws_control.send(JSON.stringify({ action: 'activateConeMode', coneNumber: coneNumber }));
    console.log('Activating cone mode with cone number:', coneNumber);
    // Update the UI to reflect the status of cone mode
    $('#cone-mode-active').text('Active (Cone ' + coneNumber + ')');
}

function deactivateConeMode() {
    // TODO: Implement the logic to send request to backend
    // Example: ws_control.send(JSON.stringify({ action: 'deactivateConeMode' }));
    console.log('Deactivating cone mode');
    // Update the UI to reflect the status of cone mode
    $('#cone-mode-active').text('Inactive');
}
