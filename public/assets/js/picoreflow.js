...
function toggleConeLight(isActive) {
    const coneLight = document.getElementById('coneLight');
    if (isActive) {
        coneLight.classList.add('light-on');
        coneLight.classList.remove('light-off');
    } else {
        coneLight.classList.add('light-off');
        coneLight.classList.remove('light-on');
    }
}
...
<!-- In your JavaScript -->
document.getElementById('activate-cone-mode').addEventListener('click', function () {
    const activate = !ovenWatcher.cone_mode;
    const coneTemperature = document.getElementById('cone-temperature').value;
    // Send a POST request to your API endpoint to activate/deactivate cone mode
    // Include the 'activate' and 'coneTemperature' data
    // Handle the response accordingly
});


// Event Listener for Set Cone Button
document.getElementById('setConeButton').addEventListener('click', function() {
    var coneNumber = document.getElementById('coneNumberSelect').value;
    // AJAX call or WebSocket to send this to the Python backend
    // Example AJAX call (update with your actual URL and method)
    fetch('/set-oven-temp', {
        method: 'POST',
        body: JSON.stringify({ coneNumber: coneNumber }),
        headers: {
...
// Function to handle timer updates from backend
function updateTimerDisplay(extendedTime) {
    document.getElementById('timerDisplay').innerText = extendedTime;
}

// WebSocket listener for timer updates
...
// Function to request timer start/stop
function toggleTimer(start) {
    var message = { type: 'toggleTimer', start: start };
    ws_control.send(JSON.stringify(message));
}
